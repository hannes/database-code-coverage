...
public class MultipleTableDatabaseExampleTest {
  ...
  private static final Logger LOG = LoggerFactory
      .getLogger(MultipleTableDatabaseExampleTest.class);
  ...
  public void initiateTestDataset() throws Exception {
    ...
    example.insertAccountInTheDatabase(ACCOUNT_ONE, new BigDecimal(100),
        DATE_FORMAT.parse("2016-11-08"), "V");
    example.insertAccountInTheDatabase("NL32AX6", new BigDecimal(200),
        DATE_FORMAT.parse("2016-11-07"), "W");
    example.insertAccountInTheDatabase("DE12ZZ4", new BigDecimal(300),
        DATE_FORMAT.parse("2016-11-06"), "X");
    example.insertAccountInTheDatabase("PL00AB8", new BigDecimal(400),
        DATE_FORMAT.parse("2016-11-05"), "Y");
    example.insertAccountInTheDatabase("FR12DE9", new BigDecimal(400),
        DATE_FORMAT.parse("2016-11-04"), "Z");
    example.insertAccountInTheDatabase("BE34FG7", new BigDecimal(300),
        DATE_FORMAT.parse("2016-11-03"), "Z");
    example.insertAccountInTheDatabase("NL56HX6", new BigDecimal(500),
        DATE_FORMAT.parse("2016-11-02"), "V");
  ...
  /**
   * This testcase is added for 'extra' coverage, but is not required for the
   * 'old' code coverage measurement
   */
  ...
  @Test
  public void getPaymentAccountsForAccountHavingOnlyPaymentAccountsTest()
      throws Exception {
    LOG.info(
        "Testing get payment accounts for account having only payment accounts");
    MultipleTableDatabaseExample example = new MultipleTableDatabaseExample();
  ...
  // Note: this testcase should increase the code coverage as it also checks the
  // substring() part of the query
  @Ignore
  @Test
  public void getPaymentAccountsForAccountHavingAlsoPaymentAccountsTest()
      throws Exception {
    LOG.info(
        "Testing get payment accounts for account having also payment accounts");
    MultipleTableDatabaseExample example = new MultipleTableDatabaseExample();
  ...
  @Test
  public void stupidEatingExceptionsTest() {
    try {
      new MultipleTableDatabaseExample().getConn().createStatement()
          .executeQuery("SELECT * FROM contract;").next();
    } catch (Exception e) {
      // e.printStackTrace();
    }
